...
class Alliance(object):
	...
	def __al_help_request(self, action_id):
		logging.debug("help action id : %s " % action_id)
		self.get(command="al_help_request", key0=action_id)
	
	def alliance_help_request(self):
		for action_id, action_detail in self.__data.get_all_actions().items():
			...
			if action.basic.action_type == Enum.ActionType.BuildUpgrade:
				self.__al_help_request(action_id=action_id)
			else:
