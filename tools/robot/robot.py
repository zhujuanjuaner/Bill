...
class Robot(object):
	def __init__(self, sid: int, uid: int = 0, ksid: int = 0):
		...
		self.lord = module.Lord(user_data=self.data, protocol=self.protocol)
		self.building = module.Building(user_data=self.data, protocol=self.protocol)
		self.al = module.Alliance(user_data=self.data, protocol=self.protocol)
