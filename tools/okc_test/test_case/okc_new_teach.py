...
def guide():
	...
	while True:
		...
		if okc_ui_test.find_ui_node(dialogue):
			okc_ui_test.click_by_node_name(view="ViewGuide", node_name=dialogue, can_change=True)
			new_teach_step += 1
			...
			okc_ui_test.click_by_node_name(view="ViewGuide", node_name=arrow, can_change=True)
			new_teach_step += 1
			...
			okc_ui_test.click_by_node_name(view="ViewCivicCenterUpgradePop", ui_node_name=collect_btn)
			okc_ui_test.get_screenshots(image_name="step_%s" % new_teach_step, save_path=image_path)
			failed_count = 0
		elif okc_ui_test.find_ui_node(view_vip_daily_pop) and okc_ui_test.find_ui_node(ok_btn):
			okc_ui_test.get_screenshots(image_name="step_%s" % new_teach_step, save_path=image_path)
			okc_ui_test.click_by_node_name(view="ViewVipDailyPop", node_name=ok_btn)
		else:
			failed_count += 1
