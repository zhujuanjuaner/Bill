...
class Map(object):
	...
	def get_march_time(self, march_type, target: int, troops=None, with_dragon=False):
		...
		return max(int((distance / speed) * game_table.get_factor(Enum.GameBasicFactor.MarchBaseSpeed)) + 1, 5)
	...
	def march_scout(self, target: int):
		...
		return self.__request.march_scout(cost_time=cost_time, pos=target, cost_gem=0, cost_rss=cost_gold)
